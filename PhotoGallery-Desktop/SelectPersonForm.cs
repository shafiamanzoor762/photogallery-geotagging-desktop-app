...
namespace PhotoGallery_Desktop
{
    ...
    {
        public Form previous_form;
        public string source_id;
        List<Person> persons;
        Logic l = new Logic();

        public SelectPersonForm(Form previousForm, string sourceId, List<Person> person)
        {
            InitializeComponent();
            this.previous_form = previousForm;
        ...
        private async void PictureBox_Click(object sender, EventArgs e)
        {
            ...
            //}
                ...
                {
                    // tagString = "person_id:5;Ali"
                    string[] parts = tagString.Split(';');
                    if (parts.Length > 0)
                    {
                        string[] keyValue = parts[0].Split(':'); // keyValue[0] = "person_id", keyValue[1] = "5"
                        if (keyValue.Length == 2 && keyValue[0] == "person_id")
                        {
                            string personId = keyValue[1];
                            // You now have the personId here

                            // Show it for testing
                            MessageBox.Show("Clicked Person ID: " + personId);

                            // 👉 Example: move selected images from SecondScreen here
                         string result = await l.MoveImagesToPersonFolder(source_id, persons, personId);
        ...
        private async void SelectPersonForm_LoadAsync(object sender, EventArgs e)
        {
            try
            {
